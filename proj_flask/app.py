...
# do it after DB configuration which is step 03
from models import Student, University, group_chat_table
from flask import Flask, jsonify, request
# this is to tell that we are using ORM
...
app.config['SQLALCHEMY_DATABASE_URI'] = ''  # INSERT DATABASE URL here
...
def createIntRecord():
    ...
    try:
        new_int_stu_record = Student(
            chat_id=telegram_chat_ID, name=user_real_name, tele_username=telegram_username, faculty=faculty)
...
# This is to populate the University table with the records from the csv file.
@app.route('/populateUniTable/', methods=['POST'])
def populateUniTable():
    with open('Partner_Universities_Overview_List_Fall_2020_1_converted.csv', 'r') as f:
        reader = csv.DictReader(f)
        try:
            for records in reader:
                uni_record = University(
                    name=records['Partner University'], country=records['Country'])
